...
export default function PlaylistPanel() {
  ...
  return (
    <div className="shrink-0 flex flex-col border-t border-cream-300">
      ...
      {/* ── Track list ─────────────────────────────────────────────────────── */}
      <div className="flex flex-col overflow-y-auto" style={{ maxHeight: '140px' }}>
          ...
          audioTracks.map((track, idx) => {
            ...
            return (
              ...
              >
                ...
                {/* File name */}
                <span className={`flex-1 text-[10px] truncate ${isActive ? 'text-ink-900 font-medium' : 'text-ink-600'}`}>
      ...
      {/* ── Box-mode preview (when mode is 'box' and tracks exist) ─────────── */}
      {playlistMode === 'box' && activeTrack && (
        <div className="mx-3 mb-2 mt-1 p-2 border border-cream-300 bg-cream-200 flex flex-col gap-0.5">
          ...
          <div className="flex items-center gap-2 text-[9px] text-ink-400">
            ...
            {nextTrack && nextTrack.id !== activeTrack.id && (
              <>
                ...
                <span className="truncate max-w-[100px]">{nextTrack.fileName.replace(/\.[^.]+$/, '')}</span>
              </>
            )}
          </div>
        </div>
