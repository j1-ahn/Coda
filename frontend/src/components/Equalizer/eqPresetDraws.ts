/**
 * EQ preset draw functions.
 *
 * Each drawXxx is a frame renderer that paints directly to the canvas. The
 * DRAW_FNS map routes `preset.id` → draw function; applyReactMode runs after
 * the main draw to composite the beat/flash/ripple overlay.
 *
 * Split from EQCanvas.tsx — the canvas host component stays lean; this module
 * owns the ~1,100 lines of preset rendering.
 */
...
// ---------------------------------------------------------------------------
export const drawRadial: DrawFn = (ctx, w, h, data, _t, _s, preset) => {
  ctx.clearRect(0, 0, w, h);
...
// ---------------------------------------------------------------------------
export function applyReactMode(
  ctx: CanvasRenderingContext2D,
  w: number, h: number,
  data: AudioAnalyserData,
  time: number,
